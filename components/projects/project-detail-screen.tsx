...
import { setActiveProject } from "@/lib/store/slices/projectsSlice";
...
export function ProjectDetailScreen({ project }: { project: ProjectDetail }) {
  ...
  const { activeMilestone, totalBudget, spent, budgetPercent } = useMemo(() => {
    const sourceProject = activeProject ?? project;
    const total = Number(sourceProject.totalBudget);
    const used = Number(sourceProject.spent);

    return {
      activeMilestone: sourceProject.milestones.find(
        (milestone) => milestone.status === "ACTIVE",
      ),
      totalBudget: total,
      spent: used,
      budgetPercent: total === 0 ? 0 : Math.round((used / total) * 100),
    };
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm text-slate-600">
                ...
                <span className="inline-flex items-center gap-1.5">
                  ...
                  {activeProject.customer.name}
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <Clock3 className="size-4 text-teal-700" />
                  {dateFormat.format(activeProject.startDate)} to{" "}
                  {dateFormat.format(activeProject.estimatedEndDate)}
                </span>
              </div>
            </div>
          </div>
      ...
      <Tabs defaultValue="overview" className="flex-col gap-4">
        ...
        <TabsContent value="overview" className="space-y-4">
          <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
            <SectionCard
              ...
              description="Live metadata, customer assignment, and delivery timing."
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <DetailItem
                  label="Status"
                  value={formatStatus(activeProject.status)}
                  tone={statusTone(activeProject.status)}
                />
                <DetailItem
                  label="Customer"
                  value={activeProject.customer.name}
                />
                <DetailItem
                  label="Site Manager"
                  value={activeProject.siteManager?.name ?? "Unassigned"}
                />
                <DetailItem label="Location" value={activeProject.location} />
                <DetailItem
                  label="Start Date"
                  value={dateFormat.format(activeProject.startDate)}
                />
                <DetailItem
                  label="Estimated End"
                  value={dateFormat.format(activeProject.estimatedEndDate)}
                />
              </div>
            </SectionCard>
            ...
            <SectionCard
              title="Budget"
              description="Spend versus contract value and current burn rate."
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  ...
                  <span>
                    {currency.format(Math.max(totalBudget - spent, 0))}{" "}
                    remaining
                  </span>
                </div>
              </div>
            ...
            <SectionCard
              ...
              description="The single milestone currently in progress."
            >
              {activeMilestone ? (
                <div className="space-y-2 rounded-2xl border border-teal-200 bg-teal-50/70 p-4">
                  ...
                  <p className="text-sm text-slate-600">
                    Target date: {dateFormat.format(activeMilestone.targetDate)}
                  </p>
                  <StatusPill tone="warning">
                    {formatStatus(activeMilestone.status)}
                  </StatusPill>
                </div>
              ) : (
        ...
        <TabsContent value="customer" className="space-y-4">
          <SectionCard
            ...
            description="Read-only requirements captured from quoting."
          >
            {Array.isArray(activeProject.requirements) &&
            activeProject.requirements.length > 0 ? (
              <ul className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {activeProject.requirements.map((requirement, index) => (
                  <li
                    key={`${formatRequirement(requirement)}-${index}`}
                    className="rounded-2xl border border-border bg-background p-4 text-sm text-slate-700 shadow-sm"
                  >
        ...
        <TabsContent value="activity" className="space-y-4">
          <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
            <SectionCard
              title="Milestones"
              description="Timeline, photos, and tradie schedule counts."
            >
              <div className="space-y-4 border-l border-border pl-5">
                {activeProject.milestones.map((milestone) => {
                  const isDone = milestone.status === "DONE";
                  const isActive = milestone.status === "ACTIVE";
                  const scheduleCount = activeProject.tradieSchedules.filter(
                    (schedule) => schedule.milestoneId === milestone.id,
                  ).length;

                  const needsReplacement = activeProject.tradieSchedules.some(
                    (schedule) =>
                      schedule.milestoneId === milestone.id &&
                  ...
                  return (
                    <div key={milestone.id} className="relative pb-5 last:pb-0">
                      <div
                        className={
                          isActive
                            ? "absolute left-[-1.55rem] top-1.5 size-3 rounded-full border-2 border-white bg-amber-500 shadow-[0_0_0_4px_rgba(245,158,11,0.2)]"
                            : isDone
                              ? "absolute left-[-1.55rem] top-1.5 size-3 rounded-full border-2 border-white bg-emerald-500"
                              : "absolute left-[-1.55rem] top-1.5 size-3 rounded-full border-2 border-white bg-slate-300"
                        }
                      />
                      <div
                        className={
                          needsReplacement
                            ? "rounded-2xl border border-red-300 bg-red-50/60 p-4"
                            : isActive
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              Target: {dateFormat.format(milestone.targetDate)}
                            </p>
            ...
            <SectionCard
              title="Site Updates"
              description="Chronological notes from the site team."
            >
              <div className="space-y-4">
                {activeProject.siteUpdates.length > 0 ? (
                  activeProject.siteUpdates.map((update) => (
                    <article
                      key={update.id}
        ...
        <TabsContent value="variations" className="space-y-4">
          <SectionCard
            title="Variations"
            description="Scope changes with approval dates, delay impact, and cost movement."
          >
            <div className="space-y-4">
              {activeProject.variations.length > 0 ? (
                activeProject.variations.map((variation) => (
                  <div
                    key={variation.id}
                    className="rounded-2xl border border-border bg-background p-4 shadow-sm"
                  >
                    <div className="grid gap-3 lg:grid-cols-[1.6fr_0.8fr_0.8fr_0.7fr_0.7fr] lg:items-center">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Requested {dateFormat.format(variation.requestedDate)}
                        </p>
                      </div>
                      <div className="text-sm text-slate-700">
                        {currency.format(Number(variation.cost))}
                      </div>
                      <div className="text-sm text-slate-700">
...
function DetailItem({
  ...
  return (
    <div className="rounded-2xl border border-border bg-background p-4 shadow-sm">
      ...
      <p
        className={`mt-2 font-heading text-lg font-semibold ${tone === "warning" ? "text-amber-700" : tone === "danger" ? "text-red-700" : tone === "success" ? "text-emerald-700" : "text-slate-950"}`}
      >
        {value}
      </p>
    </div>
