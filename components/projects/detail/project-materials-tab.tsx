...
import { CheckCircle, Clock, MinusCircle, PenSquare, Plus, Search } from "lucide-react";
...
function toneForMaterialStatus(status: "Yes" | "Partial" | "Pending" | "Not Yet" | "No") {
    ...
    <Card className="border-border/80 bg-white shadow-sm transition-all hover:shadow-md rounded-xl">
      <CardHeader className="border-b border-border/60 pb-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <CardTitle className="text-[12px] font-bold uppercase tracking-[0.06em] text-muted-foreground">Selected Materials from Catalogue</CardTitle>
      ...
      <CardContent className="space-y-4 pt-0 px-0 pb-5">
          ...
          <table className="w-full min-w-[960px] text-[13px]">
            <thead>
              <tr className="border-b border-border text-left text-[11px] font-semibold uppercase tracking-[0.04em] text-muted-foreground bg-slate-50">
                ...
                <th className="py-2.5 px-[14px] whitespace-nowrap">Specification</th>
                <th className="py-2.5 px-[14px] whitespace-nowrap">Qty</th>
                <th className="py-2.5 px-[14px] whitespace-nowrap">Unit Cost</th>
                <th className="py-2.5 px-[14px] whitespace-nowrap">Total</th>
                <th className="py-2.5 px-[14px] whitespace-nowrap">Ordered</th>
                <th className="py-2.5 px-[14px] whitespace-nowrap">Delivered</th>
                <th className="py-2.5 px-[14px] whitespace-nowrap text-right">Action</th>
              </tr>
            </thead>
            <tbody>
              {filteredItems.map((item) => (
                <tr key={item.id} className="border-b border-slate-100 align-middle transition-colors hover:bg-slate-50/80 cursor-pointer">
                  <td className="py-3 px-[14px] text-slate-900">{item.category}</td>
                  <td className="py-3 px-[14px]">{item.product}</td>
                  <td className="py-3 px-[14px] text-slate-600">{item.specification}</td>
                  <td className="py-3 px-[14px]">{item.quantity}</td>
                  <td className="py-3 px-[14px]">{item.unitCost}</td>
                  <td className="py-3 px-[14px] font-medium text-slate-900">{item.totalCost}</td>
                  <td className="py-3 px-[14px]">
                    <StatusPill tone={toneForMaterialStatus(item.orderedStatus)}>{item.orderedStatus}</StatusPill>
                  </td>
                  <td className="py-3 px-[14px]">
                    <StatusPill tone={toneForMaterialStatus(item.deliveredStatus)}>{item.deliveredStatus}</StatusPill>
                  </td>
        ...
        <div className="mx-4 mt-3 flex flex-wrap items-center justify-between gap-2 rounded-lg bg-slate-50 px-4 py-3">
          <div className="text-[13px]">
            <b>Total Materials Cost:</b> <span className="ml-1 text-base font-extrabold text-teal-600">$90,060</span>
          </div>
          <div className="flex gap-1.5">
             <StatusPill tone="success"><CheckCircle className="mr-1 size-3" />5 Delivered</StatusPill>
             <StatusPill tone="warning"><Clock className="mr-1 size-3" />1 Partial</StatusPill>
             <StatusPill tone="neutral"><MinusCircle className="mr-1 size-3" />2 Pending</StatusPill>
          </div>
        </div>
      </CardContent>
    </Card>
