...
import { selectProjectDetailUiState, setProjectDetailTab } from "@/lib/store/slices/projectsSlice";
...
export function ProjectDetailTabs({ project }: { project: ProjectDetail }) {
  ...
  const selector = useMemo(() => selectProjectDetailUiState(project.id), [project.id]);
  ...
  return (
    <section className="space-y-4">
      <ProjectTabNavigation
        ...
        onTabChange={(tab) => dispatch(setProjectDetailTab({ projectId: project.id, tab }))}
      />

      {detailUi.activeTab === "overview" ? <ProjectOverviewTab project={project} /> : null}
      {detailUi.activeTab === "milestones" ? <ProjectMilestonesTab project={project} /> : null}
      {detailUi.activeTab === "materials" ? <ProjectMaterialsTab /> : null}
      {detailUi.activeTab === "payments" ? <ProjectPaymentsTab /> : null}
      {detailUi.activeTab === "updates" ? <ProjectSiteUpdatesTab project={project} /> : null}
      {detailUi.activeTab === "tradies" ? <ProjectTradiesTab project={project} /> : null}
      {detailUi.activeTab === "workers" ? <ProjectWorkersTab /> : null}
      {detailUi.activeTab === "quotes" ? <ProjectQuotesTab /> : null}
      {detailUi.activeTab === "variations" ? <ProjectVariationsTab project={project} /> : null}
    </section>
  );
}
