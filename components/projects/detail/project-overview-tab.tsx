...
function DetailItem({
  ...
  return (
    <div className="rounded-[10px] border border-border bg-slate-50 p-3">
      <p className="text-[10px] font-bold uppercase tracking-[0.06em] text-muted-foreground mb-0.5">
        {label}
      </p>
...
export function ProjectOverviewTab({ project }: { project: ProjectDetail }) {
  ...
  const contractWithVariations = currency.format(totalBudget + variationCost);
          ...
          <CardContent className="pt-5">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
              <div className="grid gap-3 grid-cols-2">
                ...
                />
                <DetailItem
                  label="Including Variations"
                  value={contractWithVariations}
                  valueClass="text-[#E8730C]"
                />
              </div>
              <DetailItem
                label="Start Date"
              ...
              <DetailItem
                ...
                value={dateFormat.format(project.estimatedEndDate)}
              />
              <DetailItem
                label="Site Manager"
                value={project.siteManager?.name ?? "Unassigned"}
              />
              <DetailItem
                label="Architect"
                value={projectOverviewMock.architect}
              />
              <DetailItem label="Building Type" value={project.buildingType} />
              <DetailItem
                label="Lot Size"
                value={`${Number(project.lotSize ?? "0.0").toLocaleString()} m²`}
              />
              <DetailItem label="Council" value={project.council} />
              <DetailItem
                label="Certifier"
                value={projectOverviewMock.certifier}
              />
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <Button
                size="sm"
                variant="outline"
                className="h-8 rounded-lg px-2.5 text-[11.5px] font-semibold text-green-700 border-green-200 bg-green-50 hover:bg-green-100 hover:text-green-800"
              >
        ...
        <LineChartCard
          ...
          action={
            <Button
              size="sm"
              variant="outline"
              className="h-8 rounded-lg px-3 text-[11.5px] font-semibold"
            >
              ...
              {activityLog.map((item) => (
                <article key={item.id} className="relative pb-5 last:pb-0">
                  ...
                  <p className="mt-1.5 text-[11px] text-muted-foreground font-medium">
                    ...
                    {dataTimeFormat.format(new Date(item.createdAt))}
                  </p>
                </article>
              ))}
            </div>
